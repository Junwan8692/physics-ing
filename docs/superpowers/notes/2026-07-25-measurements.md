...
## 부록 — 스크롤 관성 기본값 (2026-07-26 추가)

증상: 에피소드 미리보기에서 출렁임이 단일 이미지일 때보다 각져 보임.

원인은 격자가 아니라 **입력 세기**였다. `LivePreview` 는 `[pointer, auto]` 를 쓰는데
`EpisodePreview` 는 `[pointer, scroll, auto]` 라 스크롤 어댑터가 추가된다.

### 1. 구동 세기별 접힘 한계 (웹툰 프리셋 4종 × 크롭 3종 × 600틱, 뒤집힌 삼각형이 있던 틱 수)
...
### 2. 옛 기본값이 내던 세기 (gain 0.0015, 평활 0.04s)

| 스크롤 속도 | 최대 가속도 |
|---|---|
| 800 px/s (부드럽게) | 7.23 |
| 2000 px/s (보통 플릭) | **8.00 — 23프레임 포화** |
| 4000 px/s | **8.00 — 32프레임 포화** |

일반적인 스크롤이 엔진 클램프 최대치에 붙어 있었다. 안전선의 4배.

### 3. 평활이 진짜 지렛대였다

가속도는 위치의 2차 미분이라 스크롤 시작 순간의 저크가 그대로 튄다.

| gain | 평활 | 천천히 | 보통 | 빠름 | 플릭 | 세게 | 동적 범위 |
|---|---|---|---|---|---|---|---|
| 0.0015 | 0.04s | 3.62 | 8.00 | 8.00 | 8.00 | 8.00 | **2.2배** |
| 0.0015 | 0.12s | 1.73 | 3.90 | 7.79 | 8.00 | 8.00 | 4.6배 |
| 0.0005 | 0.20s | 0.36 | 0.81 | 1.62 | 3.60 | 7.19 | 20배 |
| **0.0004** | **0.25s** | **0.23** | **0.52** | **1.04** | **2.32** | 4.64 | **20배** |

평활만 0.04 → 0.25s 로 늘려도 동적 범위가 2.2배 → 20배가 된다. 클램프만 조이면
보통 읽기부터 전부 포화되어 모든 스크롤이 같은 세기로 느껴진다.

### 확정된 기본값

```
DEFAULT_SCROLL_GAIN              = 0.0004
DEFAULT_SCROLL_SMOOTHING_SECONDS = 0.25
DEFAULT_SCROLL_MAX_ACCELERATION  = 2      ← 안전선. 아무리 세게 튕겨도 안 접힌다
```

읽는 동안 은은하고 플릭에서 눈에 띄는 게 의도다. 스크롤 내내 세게 출렁이면 읽기 방해다.
`EpisodePreview` 에 반응 세기·평활 슬라이더를 노출해 두었다 — 눈으로 맞추는 값이다.
