...
## 부록 — `GRID_K` 상향 검토 (2026-07-25 추가)

"격자를 더 촘촘하게 하면 더 부드러워지나?"에 대한 실측.

크롭 4종(가슴 494×394, 머리카락 330×680, 전신 922×1622, 정사각 400×400) × 웹툰 프리셋 4종,
`localAcceleration` 크기 ~1.7(스크롤 관성 수준), 600틱, 뒤집힌 삼각형이 있던 틱 수.

| K | 정점(가슴) | hair | cloth | chest | cheek | 최대 변위 |
|---|---|---|---|---|---|---|
| **25 (현재)** | 832 | 0~5 | **0** | **0** | **0** | 13px |
| 28 | 1,044 | 4~12 | 0~1 | 1~3 | 0 | 13px |
| 30 | 1,209 | 4~30 | 0~1 | 1~2 | 0 | 13px |
| 33 | 1,428 | 8~99 | 4~5 | 3~6 | 5~33 | 14px |
| 40 | 2,091 | 169~213 | 7 | 6~112 | 103~151 | 16px |

**결론: 올리지 않는다.**

1. 뒤집힘이 K에 대해 **단조 증가**한다. K=25는 사실상 깨끗하고(전신+hair만 5/600), K=28부터 네 프리셋 모두에서 나타난다
2. **최대 변위가 거의 안 변한다** (13 → 16px). 정점을 2.5배 쓰고 안정성을 잃으면서 얻는 게 없다
3. 벤더 프리셋 `shivery`(fluctuation 30)는 K=25에서도 218~375틱 뒤집힌다. 스펙 §9의
   `tremorStrength ≤ 0.223` 상한이 왜 필요한지 보여주는 사례 — 우리 프리셋 4종은 전부 그 아래다

체감 뭉툭함의 원인은 물리 격자가 아니라 **저작 오버레이**였다. `OVERLAY_SAMPLES = 128`이
종횡비를 무시한 양축 고정이라 1280×5120에서 셀이 10×40px이 됐다. 표본 격자를 버리고
Canvas 2D 경로(둥근 캡 폴리라인)로 직접 그리도록 교체했다 — 스트로크가 픽셀 공간에서
반지름 `(size/2)·short`인 진짜 원이라 정확히 같은 모양이 나오고, 안티에일리어싱은 캔버스가 한다.
